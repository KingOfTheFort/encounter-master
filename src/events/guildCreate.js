...
module.exports = async (client, guild) => {
  ...
  client.logger.info(`Encounter has joined ${guild.name}`);
  ...
  // Self-assign color
  try {
    const EncounterColor = guild.roles.cache.find(r => r.name === '#Seagrass');
    if (EncounterColor) await guild.me.roles.add(EncounterColor);
  } catch (err) {
    client.logger.error(err.message);
  }
